...
**v2 is now a REAL invocable skill -- `/construct-ui-skill` (short form)
is the default, superseding the old v1 plugin.**

Follow-up to the previous entry's `help`/`?` addition: testing it live
(invoking `construct-ui-skill:construct-ui-skill` via the Skill tool)
revealed that name resolves to the OLD v1 plugin (cached at
`C:\Users\<user>\.claude\plugins\cache\crestron-construct-skills\...`),
not this project's own SKILL.md -- v2 was never actually invocable, only
read/followed directly by the assistant. Researched the real mechanism
(delegated to the claude-code-guide agent rather than guessed): a
project-level skill at `.claude/skills/<name>/SKILL.md` is auto-discovered
and takes precedence over a same-named PLUGIN skill on the short
invocation form (`/construct-ui-skill`); the plugin remains reachable only
via its full namespaced form (`/construct-ui-skill:construct-ui-skill`).
User: "we can deprecate the v1 skill" -- confirmed as an explicit decision,
not assumed.

Moved `skills/construct-ui-skill/SKILL.md` -> `.claude/skills/construct-
ui-skill/SKILL.md` (`git mv`, history preserved) -- this is now the
canonical, only copy. Updated the "Status of this document" section to
...
**Added a `help`/`?` short-circuit to `skills/construct-ui-skill/SKILL.md`.**
User: "can a user type ? or type help and get a list of supported
features in the skill?" New "Help" section, placed BEFORE Step 0 so it
never touches a file or asks for context first -- triggers on `help`/`?`
as the skill's own invocation argument, or as the user's literal next
message once the skill is active, and replies with a static, hand-
maintained feature list (kept in sync with this document's own `##`
section headers) instead of proceeding. Explicitly a STATIC list, not
generated by inspecting the codebase -- documented as needing the same
manual-upkeep discipline every other claim in this file already gets.
