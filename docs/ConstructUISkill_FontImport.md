...
**Correction to the original assumption: a full Construct restart is not what's actually required.** `ThemeAndFontLoaderCmd` (same live-scan logic as the Update handler) is:
- invoked automatically on **project open** — `OpenProjectHandlerHelper.cs`: *"Font data is populated asynchronously by ThemeAndFontLoaderCmd... via OpenMostRecentSolution"*
- exposed as a directly callable client endpoint — `PageDesigner.Server\EndPoints\PageDesignerEndpoint.cs`: `GetCommand<ThemeAndFontLoaderCmd>`

So the real requirement is most likely **"close and reopen the project,"** not "close and reopen Construct" — a materially lighter ask. This is traced from source (the trigger points, not a guess), but **not yet confirmed by clicking through the live UI** to find the exact minimal action a user needs to take. Do that live check before the skill tells users anything more disruptive ("restart the whole application") than is actually necessary — a wrong "you must restart Construct" instruction is a worse user experience than a slightly-too-permissive one.
