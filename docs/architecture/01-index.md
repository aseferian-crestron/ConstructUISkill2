...
| Themes — `customvstheme` per-component-type custom-mode style properties | **Stage 1 done** (targetProperty-backed properties only — colors/borders/label/icon styling) — see `generator/style.py`, live-verified 2026-09-11 | 7 | `generator/style.py`, `component-context.json`'s `classToVariableMapping`, `customThemeStyleMixins.ts` (client-side runtime, not file-format-relevant) |
