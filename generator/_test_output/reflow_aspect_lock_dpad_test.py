...
# 200px target -- so stack_rows scales the floor down (see its docstring) rather than
# skipping the block. The layout assertions below
# are unchanged and still pass, which is the point: relaxation degrades to today's
# behavior, it doesn't lose the block.
assert len(result.warnings) == 1 and "can't honor every component's minimum size" in result.warnings[0], (
